...
Simple program to demonstrate the concept of steady state
Association and Disociation of Molecules, confiurable stbaility and cocentration
...
class rView(arcade.Window):
    ...
    def __init__(self):
        ...
        super().__init__(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE, resizable=True, update_rate=1/20)
...
Leertast - Pause """
    ...
    def setup(self, width=None, height=None,concX=None, concY=None):
        ...
        for _ in range(self.concX):
            ...
            x = particle(texture=self.xTexture, center_x=ppos[0], center_y=ppos[1], motion=3, name="X", mass=self.massX)
            self.atomXList.append(x)
            self.completeList.append(x)
        
        for _ in range(self.concY):
            ppos = self.placeParticle(self.yAtomRadius)
            y = particle(texture=self.yTexture,  center_x=ppos[0], center_y=ppos[1], motion=3, name="Y", mass=self.massY)
    ...
    def on_update(self, delta_time):
        ...
        while self.timer >= fixed_delta_time:
            ...
            """if self.timer > 1:
            if delta_time > (1.1 / self.hertz):
                print(f"Cannot sustain freq, delta_time is {delta_time}, should be {1 / self.hertz}")
                self.hertz -= 10
                print (f"Lowering Hertz to {self.hertz}")
                self.set_update_rate(1/self.hertz)
            self.timer = 0"""
